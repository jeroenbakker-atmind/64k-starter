...
use song::format::{DEVICE_NAMES, MidiEvent, Song, Track, decode, encode};
...
fn song_from_parsed(parsed: &song::format::ParsedSong) -> Song {
...
fn main() {
    ...
    if !verify {
        ...
        if want_wav {
            ...
            let mut mix = song::render::render(&parsed);
            song::render::normalize(&mut mix);
            let mix_path = Path::new(&export_dir).join("original-mix.wav");
            song::render::write_wav_at(
            ...
            for (ti, t) in parsed.tracks.iter().enumerate() {
                ...
                let mut solo = song::render::render_solo(&parsed, ti);
                song::render::normalize(&mut solo);
