...
fn main() {
    ...
    if want_wav {
        ...
        song::render::write_wav_at(&mix_path.to_string_lossy(), &mix, SAMPLE_RATE as u32);
        println!(
            "wrote {} ({:.2}s mono wav)",
            mix_path.display(),
        ...
        for (name, tracks) in stems {
            ...
            for &ti in *tracks {
                let solo = song::render::render_solo(&parsed, ti);
                if buf.is_empty() {
                    buf = solo;
                } else {
            ...
            song::render::normalize(&mut buf);
            let stem_path = Path::new(&export_dir).join(format!("{stem_base}.{name}.wav"));
            song::render::write_wav_at(&stem_path.to_string_lossy(), &buf, SAMPLE_RATE as u32);
