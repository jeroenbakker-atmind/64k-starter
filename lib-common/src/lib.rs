...
impl Default for Adultery {
    fn default() -> Self {
        Adultery {
            sample_index: 1.0,
            amp_attack: env_ms(2.0),
            amp_decay: env_ms(500.0),
            amp_sustain: 1.0,
            amp_release: env_ms(3000.0),
            sample_start: 0.0,
            loop_mode: 0.5,
            loop_boundary_mode: 0.0,
            loop_start: 0.0,
            loop_length: 1.0,
            reverse: 0.0,
            ...
            filter_mod_amt: 0.5,
            mod_attack: env_ms(2.0),
            mod_decay: env_ms(50.0),
            mod_sustain: 1.0,
            mod_release: env_ms(500.0),
            voices_unisono: 0.0,
            voices_detune: 0.0,
            voices_pan: 0.5,
            master: 0.5,
...
pub fn encode(song: &Song) -> Vec<u8> {
    ...
    for (ti, t) in song.tracks.iter().enumerate() {
        ...
        out.extend_from_slice(&(ti as i32).to_le_bytes());
